...
"""
import os, sys, math, random, json, time, warnings
...
def run_experiment(arch_cfg, use_muon):
    ...
    cfg = HelixConfig(
        vocab_size=vs, d_model=d, n_columns=n_columns,
        nodes_per_column=nodes_per_column, n_heads=n_heads,
        n_loops=arch_cfg['n_loops'], seq_len=SEQ_LEN, batch_size=BATCH_SIZE,
        use_titans_memory=False, attention_mode="hybrid",
        dropout=arch_cfg['dropout'],
        attn_dropout=min(arch_cfg['dropout'] + 0.05, 0.25),
        lr=arch_cfg['lr'], weight_decay=arch_cfg['wd'],
        epochs=EPOCHS, warmup_steps=50, grad_clip=1.0,
        device="cpu", dtype="float32", use_cca=False,
    )
    ...
    train_loader = create_document_loader(
        train_texts[:N_TRAIN], tok, seq_len=SEQ_LEN, batch_size=BATCH_SIZE,
        shuffle=True, drop_last=True, lazy=True,
    )
    val_loader = create_document_loader(
        val_texts[:N_VAL], tok, seq_len=SEQ_LEN, batch_size=BATCH_SIZE,
    ...
    trainer = Trainer(
        ...
        tokenizer=tok, output_dir=out_dir, grad_accum_steps=1, use_amp=False, verbose=False,
    )
    
    # ── Build custom optimizer ─────────────────────────────────────────
    if use_muon:
        # Split: Muon for 2D matrices, AdamW for non-2D
        muon_params = []
        adamw_params = []
        ...
        # Use AdamW as the primary optimizer for scheduler purposes
        # Manually handle Muon steps
        trainer.optimizer = adamw_opt
        
        # Patch train_epoch to also step Muon
        original_train_epoch = trainer.train_epoch
        
        def patched_train_epoch(epoch):
            ...
            for batch_idx, batch in enumerate(tqdm(train_loader, desc=f"Ep{epoch}", disable=True)):
        ...
        trainer.scheduler = None  # Will be built in patched train_epoch
        
    else:
        # Pure AdamW - trainer already has it, just verify betas
    ...
    try:
        ...
        result = {
            "run": run_name, "arch": arch_cfg["name"], "optimizer": opt_name,
            "d_model": d, "n_loops": arch_cfg["n_loops"],
            "lr": arch_cfg["lr"], "wd": arch_cfg["wd"], "dropout": arch_cfg["dropout"],
            "val_ppl": val_ppl, "val_loss": val_loss, "train_loss": train_loss,
            "params": params, "time": time.time() - t0, "success": True,
        }
        print(f"  ✅ Val PPL: {val_ppl:.2f} | Train: {train_loss:.4f} | {result['time']:.1f}s")
        ...
        result = {
            ...
            "d_model": d, "n_loops": arch_cfg["n_loops"],
            "lr": arch_cfg["lr"], "wd": arch_cfg["wd"], "dropout": arch_cfg["dropout"],
            "val_ppl": float("inf"), "val_loss": float("inf"), "train_loss": float("inf"),
            "params": params, "time": time.time() - t0, "success": False, "error": str(e),
        }
...
def main():
    ...
    print(f"{'='*60}")
    print(f"3 configs × 2 optimizers = 6 runs")
    print(f"Scale: {N_TRAIN} train, {N_VAL} val, {SEQ_LEN} seq, {EPOCHS} epochs, CPU")
    print(f"{'='*60}")
    ...
    for i, r in enumerate(sorted_r):
        print(f"{i+1:<6}{r['run']:<22}{r['val_ppl']:<12.2f}{r['train_loss']:<14.4f}{r['time']:<10.1f}s")
    ...
    for arch in [c["name"] for c in ARCH_CONFIGS]:
        muon_r = next((r for r in results if r["arch"] == arch and r["optimizer"] == "muon"), None)
        adam_r = next((r for r in results if r["arch"] == arch and r["optimizer"] == "adamw"), None)
        if muon_r and adam_r:
            mp, ap = muon_r["val_ppl"], adam_r["val_ppl"]
            diff = ap - mp
            winner = "MUON" if mp < ap else "ADAMW"
            print(f"  {arch}: Muon={mp:.2f} vs AdamW={ap:.2f} | delta={diff:+.2f} → {winner}")
    
    with open("./results_muon_adamw/results_final.json", "w") as f:
        json.dump({"results": results, "sorted": sorted_r, "summary": {
