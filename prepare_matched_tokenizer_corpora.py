...
SCHEMA = "helix.matched-tokenizer-corpus.v0"
SUMMARY_SCHEMA = "helix.matched-tokenizer-preparation.v0"
...
DEFAULT_OUTPUT_ROOT = Path(
    "/home/mo/DEV/experiments/helix-lengthmax-matched-pilot-data-v0"
)
...
def write_shard(root: Path, shard_id: int, values: list[int]) -> dict[str, Any]:
...
def materialize_matched_split(
    *,
    ...
    for text in iter_parquet_texts(paths):
        ...
        for arm, tokenizer in tokenizers.items():
            ...
            while len(buffers[arm]) >= shard_tokens:
                ...
                shards[arm].append(write_shard(roots[arm], len(shards[arm]), shard))
    ...
    for arm in tokenizers:
        if buffers[arm]:
            shards[arm].append(write_shard(roots[arm], len(shards[arm]), buffers[arm]))
