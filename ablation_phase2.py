"""Phase 2: CCA warmup ablation. Inherits winning (LR=2e-3, n_loops=2) from Phase 1."""
...
SUBSET_SIZE = 1000
EPOCHS = 3
OUTPUT = "/app/phase2_results.json"
...
def main():
    ...
    # Phase 2: CCA warmup sweep on winning config
    # Approximate steps: ~900 train samples / 8 bs ≈ 113 batches/epoch
    # With n_loops=2, ~113 optimizer steps/epoch, 3 epochs ≈ 339 total steps
    # 10% ≈ 34 steps, 25% ≈ 85 steps
    runs = [
        (34, "cca10pct"),
        (85, "cca25pct"),
    ]

    for cca_warmup_steps, label in runs:
        print(f"\n{'='*60}")
        print(f"Running: {label} | cca_warmup_steps={cca_warmup_steps}")
        print(f"{'='*60}")
    ...
    for r in sorted(results, key=lambda x: x["val_loss"]):
        print(
            f"  {r['label']:12s}  cca_warmup={r['cca_warmup_steps']}  "
