"""
HelixLM 87M/3B Token Production trainer — d1024, n_columns=2, ffn=2.7, seq_len=1024, n_loops=4
Constant LR per stage, dense topology. Streaming dataset for 3B tokens.

Config (Optimized per PanGu-π + Cerebros insight):
  d_model=1024, n_columns=2, n_heads=16, ffn_expansion=2.7, seq_len=1024, n_loops=4
...
Rationale:
  ...
  - stride=512: Standard for 1024 seq_len (>50% overlap for better data utilization)
"""
import math
import json
import sys
...
# Architecture — Optimized configuration per PanGu-π + Cerebros insights
...
FFN_EXPANSION = 2.7                         # Per PanGu-π
SEQ_LEN = 1024                              # Target context length
N_LOOPS = 4                                 # Recurrent depth (proportional to seq_len)
...
OUTPUT_DIR = Path("/app/HelixLM/production_run_87M_1024")
OUTPUT_DIR.mkdir(parents=True, exist_ok=True)
LOG_FILE = OUTPUT_DIR / f"production_train_87M_1024_{RUN_TS}.log"
RESULTS_JSON = OUTPUT_DIR / f"production_results_87M_1024_{RUN_TS}.json"
...
def main():
    ...
    logger.info("=" * 70)
    logger.info("HelixLM 87M Training — d%d, cols=%d, seq=%d, loops=%d", D_MODEL, N_COLUMNS, SEQ_LEN, N_LOOPS)
    logger.info("Design: High dim > many columns (Cerebros) | FFN 2.7 (PanGu-π)")
    logger.info("=" * 70)
    ...
    final = {
        ...
        "design_principles": [
            "High dim > many columns (Cerebros insight)",
            "FFN 2.7 per PanGu-π",
            "Simple graph (n_columns=2) for faster training",
            "Recurrent depth proportional to seq_len"
        ],
