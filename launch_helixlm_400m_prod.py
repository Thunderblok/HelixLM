"""
HelixLM 400M Token Production Run
===================================

One-shot production run on the full 400M token dataset.
Uses the validated config from smoke tests.

Expected: ~12-18h on L40S or A100. Target val PPL < 80 (beat bugged baseline).
"""
...
parser.add_argument("--output_dir", type=str, default="./checkpoints_400m_prod")
parser.add_argument("--hub_model_id", type=str, default="david-thrower/helixlm-400m-prod")
...
# ── Train ────────────────────────────────────────────────────────────────
print(f"\n{'='*70}")
print("HELIXLM 400M PRODUCTION RUN")
print(f"{'='*70}")
