...
DEFAULT_ARTIFACTS_ROOT = Path(
    "/home/mo/DEV/experiments/helix-lengthmax-matched-exact-artifacts-v1"
)
...
ARMS = ("gpt2", "lengthmax")
TARGET_CAUSAL_TARGETS = 17_169_600
MAX_OPTIMIZER_STEPS = 400
...
def prepare_data(args: argparse.Namespace) -> None:
    ...
    command = [
        sys.executable,
        str(ROOT / "prepare_matched_tokenizer_exact_corpora.py"),
        "--raw-root",
        str(args.raw_root),
        "--output-root",
        str(args.data_root),
...
def run_arm(
    *, args: argparse.Namespace, identity: dict[str, str], pair_id: str, arm: str
    ...
    command = [
        sys.executable,
        str(ROOT / "run_matched_tokenizer_exact_arm.py"),
        "--arm",
        arm,
        "--pilot-pair-id",
        pair_id,
    ...
    result = {
        ...
        "arms": {
            arm: {
                ...
                "raw_bytes_seen": terminal["raw_bytes_seen"],
                "last_validation": terminal["last_validation"],
                "run_root": terminal["run_root"],
            }
            for arm, terminal in terminals.items()
        },
        "quality_comparison": "exact_bits_per_byte; perplexity is within-tokenizer only",
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--arm-order", choices=("gpt2-first", "lengthmax-first"), default="lengthmax-first")
...
def main() -> None:
    ...
    pair_id = f"matched-tokenizer-b49-s400-exact-{timestamp}"
    pair_root = args.artifacts_root / pair_id
    pair_root.mkdir(parents=True, exist_ok=False)
    preflight = {
        "schema": "helix.matched-tokenizer-exact-preflight.v1",
        "pilot_pair_id": pair_id,
        "source": identity,
