...
class NodesPerColumnTopologyTest(unittest.TestCase):
    ...
    def test_observed_counts_come_from_registered_modules_not_the_spec(self):
        graph = HelixGraph(topology_config((2, 3, 2)))
        del graph.nodes["n0"]
    ...
    def test_gate_capacity_matches_every_wired_predecessor(self):
        graph = HelixGraph(
            topology_config(
                (3, 4, 4, 3),
                lateral_p=0.8,
        ...
        gate_capacity = {
            ...
            if graph.node_meta[name][2] == "gate"
        }
        specified_capacity = {
            name: graph.node_spec[graph.node_meta[name][0]][graph.node_meta[name][1]][1][
                "n_preds"
            ]
            for name in gate_capacity
        }
        wired_predecessors = {
            name: max(1, len(graph.graph[name]))
            for name in gate_capacity
        }

        self.assertEqual(gate_capacity, wired_predecessors)
        self.assertEqual(specified_capacity, wired_predecessors)

    def test_root_gate_reserves_one_weight_for_the_raw_graph_input(self):
        graph = HelixGraph(
            topology_config(
                (2,),
                lateral_p=0.0,
        ...
        gate_name = next(
            name for name, meta in graph.node_meta.items() if meta[2] == "gate"
        )

        self.assertEqual(graph.graph[gate_name], [])
        self.assertEqual(graph.nodes[gate_name].n_preds, 1)
        output, _ = graph(torch.randn(1, 8, 32))
        self.assertEqual(tuple(output.shape), (1, 8, 32))
