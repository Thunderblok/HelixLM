"""
HF Job launcher: HelixLM 50M token smoke test.
Fast validation of the production config before burning GPU on 400M.
Expected: 30-60 min on a10g-small, target Val PPL < 120 after epoch 1.
"""
...
import trackio
trackio.init(
    project="helixlm-50m-smoke",
    run_name="helixlm-384d-cca-50Mt-smoke",
)
...
trainer = Trainer(
    ...
    tokenizer=tok,
    output_dir="./checkpoints_50M_smoke",
...
# Trackio log final results + alert
trackio.log({
...
if final_ppl < 80:
    trackio.alert("SMOKE_PASS", f"PPL={final_ppl:.2f} < 80. Ready for 400M production run.", level="INFO")
elif final_ppl < 120:
    trackio.alert("SMOKE_PROMISING", f"PPL={final_ppl:.2f} < 120. Borderline — consider tuning before 400M.", level="WARN")
else:
    trackio.alert("SMOKE_FAIL", f"PPL={final_ppl:.2f} >= 120. Do NOT proceed to 400M. Debug first.", level="ERROR")
...
# Save
os.makedirs("./checkpoints_50M_smoke", exist_ok=True)
model.save_pretrained("./checkpoints_50M_smoke/final_model")
with open("./checkpoints_50M_smoke/results.json", "w") as f:
    json.dump({
...
# Push to hub
HUB_ID = "david-thrower/HelixLM-384d-cca-50Mt-smoke"
...
print(f"\n{'='*60}")
if final_ppl < 80:
    print(f"SHIP IT! PPL={final_ppl:.2f} < 80 — GO FOR 400M")
elif final_ppl < 120:
    print(f"PRODUCTION GATE OPEN: PPL={final_ppl:.2f} < 120")
else:
    print(f"NEEDS WORK: PPL={final_ppl:.2f} — DO NOT RUN 400M")
