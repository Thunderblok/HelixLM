...
DEFAULT_ARTIFACTS = Path("/home/mo/DEV/experiments/helix-lengthmax-matched-pilot-artifacts-v0")
...
def load_and_validate_manifest(
    ...
    for item in manifest.get("shards", []):
        ...
        refs.append(ShardRef(shard_id, path, int(item["tokens"]), expected_bytes, str(item["sha256"])))
    if not refs:
...
) -> Iterator[tuple[torch.Tensor, DataOffset]]:
    offset = start
    epoch = offset.epoch
...
def iter_batches(
    sample_iter: Iterator[tuple[torch.Tensor, DataOffset]],
    *,
    batch_size: int,
) -> Iterator[tuple[dict[str, torch.Tensor], DataOffset]]:
    while True:
        ...
        yield {"input_ids": input_ids, "labels": input_ids.clone(), "attention_mask": torch.ones_like(input_ids)}, last_offset
...
def parse_args() -> argparse.Namespace:
    ...
    ap.add_argument("--max-optimizer-steps", type=int, default=110)
    ap.add_argument("--target-causal-targets", type=int, default=4_721_640)
    ap.add_argument("--batch-size", type=int, default=12)
    ap.add_argument("--grad-accum", type=int, default=7)
    ap.add_argument("--compressed-windows", type=int, default=8)
    ap.add_argument("--checkpoint-every", type=int, default=55)
    ap.add_argument("--eval-every", type=int, default=55)
...
def main() -> None:
    ...
    run_name = f"matched-b49-{args.arm}-d512-s512-k8-nl3-b12-a7-s110-{run_stamp}"
    ...
    logger = RealtimeMLflowLogger(
        ...
        experiment="helix-lengthmax-matched-model-pilot-v0",
        run_name=run_name,
        ...
        params={
            ...
            **tokenizer_court,
            ...
            "runner_mode": "matched_tokenizer_branch49_u16_v0",
        ...
        tags={
            "run_kind": "matched_tokenizer_branch49_pilot_v0",
            "pilot_pair_id": args.pilot_pair_id,
            "pilot_arm": args.arm,
    ...
    logger.log_metrics(
        {
            ...
            "train/estimated_raw_bytes_seen": (
                data_offset.raw_tokens_seen / train_manifest["tokens_per_raw_byte"]
            ),
        },
        step=step,
        phase="preflight",
    ...
    set_optimizer_lr(optimizer, base_lr=args.learning_rate, optimizer_step_number=step + 1, warmup_optimizer_steps=warmup_optimizer_steps)
    try:
        for batch, batch_offset in iter_batches(sample_iter, batch_size=args.batch_size):
            ...
            logger.log_metrics(
                {
                    ...
                    "train/estimated_raw_bytes_per_second": (
                        data_offset.raw_tokens_seen
                        / train_manifest["tokens_per_raw_byte"]
                        / elapsed
                    ),
                    "train/lr": current_lr,
            ...
            if step % args.eval_every == 0:
                ...
                with torch.no_grad():
                    for index, (val_batch, _) in enumerate(iter_batches(val_iter, batch_size=args.batch_size)):
                ...
                val_estimated_raw_bytes = (
                    (val_targets * cfg.seq_len / (cfg.seq_len - 1))
                    / val_manifest["tokens_per_raw_byte"]
                )
                val_estimated_bits_per_byte = (
                    val_loss * val_targets / math.log(2) / max(val_estimated_raw_bytes, 1)
                )
                last_validation = {
                    "loss": val_loss,
                    "ppl": perplexity(val_loss),
                    "causal_targets": float(val_targets),
                    "estimated_raw_bytes": val_estimated_raw_bytes,
                    "estimated_bits_per_byte": val_estimated_bits_per_byte,
                }
                logger.log_metrics(
                    {
                        "val/loss": val_loss,
                        ...
                        "val/estimated_raw_bytes": val_estimated_raw_bytes,
                        "val/estimated_bits_per_byte": val_estimated_bits_per_byte,
                    },
                    step=step,
                    phase="validation",
                )
